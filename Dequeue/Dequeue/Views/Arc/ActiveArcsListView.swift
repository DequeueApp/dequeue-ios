//
//  ActiveArcsListView.swift
//  Dequeue
//
//  Displays list of active (in-progress) arcs
//

import SwiftUI
import SwiftData

struct ActiveArcsListView: View {
    ...
    init() {
        ...
        _arcs = Query(
            filter: #Predicate<Arc> { arc in
                arc.isDeleted == false &&
                arc.statusRawValue == activeRawValue
            },
            sort: \Arc.sortOrder
        )
    }
    ...
    private var emptyState: some View {
        ContentUnavailableView(
            "No Active Arcs",
            systemImage: "rays",
            description: Text("Active arcs will appear here")
        )
    }
...
#Preview {
    ...
    let arc1 = Arc(title: "Q1 Goals", status: .active)
    let arc2 = Arc(title: "Team Initiatives", status: .active)
    context.insert(arc1)
    context.insert(arc2)

    return ActiveArcsListView()
